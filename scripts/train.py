...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--backend", type=str,
                   choices=["azure_openai", "codex", "codex_exec", "claude", "claude_chat", "claude_code_exec", "qwen", "qwen_chat", "minimax", "minimax_chat", "custom", "custom_chat", "openai_compatible"])
...
def load_config(args: argparse.Namespace) -> dict:
    ...
    if explicit_backend is not None:
        ...
        if backend in {"claude", "claude_chat"}:
            ...
            flat.setdefault("optimizer_backend", "custom_chat")
            flat.setdefault("target_backend", "custom_chat")
        else:
            flat.setdefault("optimizer_backend", "openai_chat")
    ...
    if backend == "custom_chat":
        if flat.get("optimizer_backend") in (None, "", "openai_chat"):
            flat["optimizer_backend"] = "custom_chat"
        if flat.get("target_backend") in (None, "", "openai_chat"):
            flat["target_backend"] = "custom_chat"
    ...
    if flat.get("optimizer_backend") == "custom_chat":
        if (
            str(flat.get("optimizer_model", "") or "").strip() in _OPENAI_DEFAULT_MODEL_SENTINELS
            and not _has_model_override("model.optimizer", "optimizer_model")
        ):
            flat["optimizer_model"] = (
                flat.get("optimizer_custom_chat_model")
                or flat.get("custom_chat_model")
    ...
    if flat.get("target_backend") == "custom_chat":
        if (
            str(flat.get("target_model", "") or "").strip() in _OPENAI_DEFAULT_MODEL_SENTINELS
            and not _has_model_override("model.target", "target_model")
        ):
            flat["target_model"] = (
                flat.get("target_custom_chat_model")
                or flat.get("custom_chat_model")
                or default_model_for_backend("custom_chat")
            )
