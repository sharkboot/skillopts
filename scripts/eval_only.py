...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--backend", type=str,
                   choices=["azure_openai", "codex", "codex_exec", "claude", "claude_chat", "claude_code_exec", "custom", "custom_chat", "openai_compatible"])
    ...
    p.add_argument("--custom_chat_base_url", type=str)
    p.add_argument("--custom_chat_api_key", type=str)
    p.add_argument("--custom_chat_model", type=str)
    p.add_argument("--custom_chat_temperature", type=float)
    p.add_argument("--custom_chat_timeout_seconds", type=float)
    p.add_argument("--custom_chat_max_tokens", type=int)
    p.add_argument("--optimizer_custom_chat_base_url", type=str)
    p.add_argument("--optimizer_custom_chat_api_key", type=str)
    p.add_argument("--optimizer_custom_chat_model", type=str)
    p.add_argument("--optimizer_custom_chat_temperature", type=float)
    p.add_argument("--optimizer_custom_chat_timeout_seconds", type=float)
    p.add_argument("--optimizer_custom_chat_max_tokens", type=int)
    p.add_argument("--target_custom_chat_base_url", type=str)
    p.add_argument("--target_custom_chat_api_key", type=str)
    p.add_argument("--target_custom_chat_model", type=str)
    p.add_argument("--target_custom_chat_temperature", type=float)
    p.add_argument("--target_custom_chat_timeout_seconds", type=float)
    p.add_argument("--target_custom_chat_max_tokens", type=int)
...
def main() -> None:
    ...
    if cli:
        if structured:
            ...
            _MAP = {
                ...
                "custom_chat_base_url": "model.custom_chat_base_url",
                "custom_chat_api_key": "model.custom_chat_api_key",
                "custom_chat_model": "model.custom_chat_model",
                "custom_chat_temperature": "model.custom_chat_temperature",
                "custom_chat_timeout_seconds": "model.custom_chat_timeout_seconds",
                "custom_chat_max_tokens": "model.custom_chat_max_tokens",
                "optimizer_custom_chat_base_url": "model.optimizer_custom_chat_base_url",
                "optimizer_custom_chat_api_key": "model.optimizer_custom_chat_api_key",
                "optimizer_custom_chat_model": "model.optimizer_custom_chat_model",
                "optimizer_custom_chat_temperature": "model.optimizer_custom_chat_temperature",
                "optimizer_custom_chat_timeout_seconds": "model.optimizer_custom_chat_timeout_seconds",
                "optimizer_custom_chat_max_tokens": "model.optimizer_custom_chat_max_tokens",
                "target_custom_chat_base_url": "model.target_custom_chat_base_url",
                "target_custom_chat_api_key": "model.target_custom_chat_api_key",
                "target_custom_chat_model": "model.target_custom_chat_model",
                "target_custom_chat_temperature": "model.target_custom_chat_temperature",
                "target_custom_chat_timeout_seconds": "model.target_custom_chat_timeout_seconds",
                "target_custom_chat_max_tokens": "model.target_custom_chat_max_tokens",
    ...
    if explicit_backend is not None:
        ...
        elif backend in {"custom", "custom_chat", "openai_compatible"}:
            cfg.setdefault("optimizer_backend", "custom_chat")
            cfg.setdefault("target_backend", "custom_chat")
        else:
            cfg.setdefault("optimizer_backend", "openai_chat")
    ...
    if backend == "custom_chat":
        if cfg.get("optimizer_backend") in (None, "", "openai_chat"):
            cfg["optimizer_backend"] = "custom_chat"
        if cfg.get("target_backend") in (None, "", "openai_chat"):
            cfg["target_backend"] = "custom_chat"
    ...
    if cfg.get("optimizer_backend") == "custom_chat":
        if (
            str(cfg.get("optimizer_model", "") or "").strip() in _OPENAI_DEFAULT_MODEL_SENTINELS
            and not _has_model_override("model.optimizer", "optimizer_model")
        ):
            cfg["optimizer_model"] = (
                cfg.get("optimizer_custom_chat_model")
                or cfg.get("custom_chat_model")
                or default_model_for_backend("custom_chat")
            )
    ...
    if cfg.get("target_backend") == "claude_code_exec":
        if (
            ...
            cfg["target_model"] = default_model_for_backend("claude_chat")
    if cfg.get("target_backend") == "custom_chat":
        if (
            str(cfg.get("target_model", "") or "").strip() in _OPENAI_DEFAULT_MODEL_SENTINELS
            and not _has_model_override("model.target", "target_model")
        ):
            cfg["target_model"] = (
                cfg.get("target_custom_chat_model")
                or cfg.get("custom_chat_model")
                or default_model_for_backend("custom_chat")
            )
    ...
    if cfg.get("optimizer_backend") == "custom_chat":
        custom_optimizer_model = (
            cfg.get("optimizer_custom_chat_model") or cfg.get("custom_chat_model")
        )
        if custom_optimizer_model:
            set_optimizer_deployment(str(custom_optimizer_model))
    if cfg.get("target_backend") == "custom_chat":
        custom_target_model = (
            cfg.get("target_custom_chat_model") or cfg.get("custom_chat_model")
        )
        if custom_target_model:
            set_target_deployment(str(custom_target_model))
    configure_custom_chat(
        base_url=cfg.get("custom_chat_base_url") or None,
        api_key=cfg.get("custom_chat_api_key") or None,
        model=cfg.get("custom_chat_model") or None,
        temperature=cfg.get("custom_chat_temperature"),
        timeout_seconds=cfg.get("custom_chat_timeout_seconds"),
        max_tokens=cfg.get("custom_chat_max_tokens"),
        optimizer_base_url=cfg.get("optimizer_custom_chat_base_url") or None,
        optimizer_api_key=cfg.get("optimizer_custom_chat_api_key") or None,
        optimizer_model=cfg.get("optimizer_custom_chat_model") or None,
        optimizer_temperature=cfg.get("optimizer_custom_chat_temperature"),
        optimizer_timeout_seconds=cfg.get("optimizer_custom_chat_timeout_seconds"),
        optimizer_max_tokens=cfg.get("optimizer_custom_chat_max_tokens"),
        target_base_url=cfg.get("target_custom_chat_base_url") or None,
        target_api_key=cfg.get("target_custom_chat_api_key") or None,
        target_model=cfg.get("target_custom_chat_model") or None,
        target_temperature=cfg.get("target_custom_chat_temperature"),
        target_timeout_seconds=cfg.get("target_custom_chat_timeout_seconds"),
        target_max_tokens=cfg.get("target_custom_chat_max_tokens"),
    )
    configure_codex_exec(
