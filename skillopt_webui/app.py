...
def validate_training_config(
    ...
    if missing_openai_roles:
        ...
        elif configured_backend in {"custom", "custom_chat", "openai_compatible"}:
            detail = (
                "\nNote: model.backend is custom_chat, but explicit optimizer_backend/"
                "target_backend values are still openai_chat."
            )
        return (
    ...
    custom_missing = []
    custom_failures = []
    custom_shared = cfg.get("custom_chat_base_url") or env.get("CUSTOM_CHAT_BASE_URL")
    for role in ("optimizer", "target"):
        if cfg.get(f"{role}_backend") != "custom_chat":
            continue
        base_url = (
            cfg.get(f"{role}_custom_chat_base_url")
            or env.get(f"{role.upper()}_CUSTOM_CHAT_BASE_URL")
            or custom_shared
        )
        if not base_url:
            custom_missing.append(role)
            continue
        if not _can_connect_to_url(str(base_url)):
            custom_failures.append(f"{role}={base_url}")
    if custom_missing:
        return (
            "❌ Model backend is not ready: missing custom_chat base_url "
            f"for {', '.join(custom_missing)}.\n"
            "Set model.custom_chat_base_url / CUSTOM_CHAT_BASE_URL, or the "
            "optimizer/target-specific custom_chat base_url."
        )
    if custom_failures:
        return (
            "❌ Model backend is not ready: cannot connect to custom_chat endpoint "
            f"for {', '.join(custom_failures)}.\n"
            "Start your OpenAI-compatible server, or set model.custom_chat_base_url / "
            "OPTIMIZER_CUSTOM_CHAT_BASE_URL / TARGET_CUSTOM_CHAT_BASE_URL to the correct URL."
        )
    return None
