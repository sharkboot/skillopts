...
|---|---|---|---|
| `model.backend` | str | `azure_openai` | Backend: `azure_openai` / `openai_chat` / `claude_code_exec` / `qwen` / `minimax` / `custom_chat` |
...
| `model.custom_chat_base_url` | str | — | Shared OpenAI-compatible `/chat/completions` endpoint for `custom_chat` |
| `model.custom_chat_api_key` | str | — | Optional shared bearer token for `custom_chat` |
| `model.custom_chat_model` | str | — | Shared served model name for `custom_chat` |
| `model.optimizer_custom_chat_*` | mixed | — | Optimizer-specific custom endpoint/API key/model/temperature/timeout/max-token overrides |
| `model.target_custom_chat_*` | mixed | — | Target-specific custom endpoint/API key/model/temperature/timeout/max-token overrides |
...
| `CUSTOM_CHAT_BASE_URL` | Shared OpenAI-compatible endpoint for `custom_chat` |
| `CUSTOM_CHAT_API_KEY` | Optional shared bearer token for `custom_chat` |
| `CUSTOM_CHAT_MODEL` | Shared served model name for `custom_chat` |
| `OPTIMIZER_CUSTOM_CHAT_BASE_URL` | Optimizer-specific custom endpoint |
| `OPTIMIZER_CUSTOM_CHAT_API_KEY` | Optimizer-specific custom API key |
| `OPTIMIZER_CUSTOM_CHAT_MODEL` | Optimizer-specific custom model name |
| `TARGET_CUSTOM_CHAT_BASE_URL` | Target-specific custom endpoint |
| `TARGET_CUSTOM_CHAT_API_KEY` | Target-specific custom API key |
| `TARGET_CUSTOM_CHAT_MODEL` | Target-specific custom model name |
