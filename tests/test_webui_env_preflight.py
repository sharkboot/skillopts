...
def test_preflight_reports_missing_custom_chat_endpoint(tmp_path, monkeypatch):
    monkeypatch.delenv("CUSTOM_CHAT_BASE_URL", raising=False)
    monkeypatch.delenv("OPTIMIZER_CUSTOM_CHAT_BASE_URL", raising=False)
    monkeypatch.delenv("TARGET_CUSTOM_CHAT_BASE_URL", raising=False)
    config_path = _write_config(
        tmp_path,
        {
    ...
    assert "missing custom_chat base_url for optimizer, target" in error


def test_preflight_reports_unreachable_custom_chat_endpoint(tmp_path, monkeypatch):
    monkeypatch.setattr(webui_app, "_can_connect_to_url", lambda _url: False)
    config_path = _write_config(
        tmp_path,
        {
            "optimizer_backend": "custom_chat",
            "target_backend": "custom_chat",
            "custom_chat_base_url": "http://127.0.0.1:9/v1",
        },
    )

    error = webui_app.validate_training_config(config_path, {})

    assert "cannot connect to custom_chat endpoint" in error
    assert "127.0.0.1:9" in error


def test_preflight_accepts_reachable_custom_chat_endpoint(tmp_path, monkeypatch):
    seen_urls = []
    monkeypatch.setattr(webui_app, "_can_connect_to_url", lambda url: seen_urls.append(url) or True)
    config_path = _write_config(
        tmp_path,
    ...
    assert webui_app.validate_training_config(config_path, {}) is None
    assert seen_urls == ["http://custom.example/v1", "http://custom.example/v1"]
