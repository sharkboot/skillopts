...
from skillopt.model import custom_chat_backend as _custom
...
def set_backend(name: str | None) -> str:
    ...
    if normalized in {"minimax", "minimax_chat"}:
        ...
        return "minimax_chat"
    if normalized in {"custom", "custom_chat", "openai_compatible"}:
        set_optimizer_backend("custom_chat")
        set_target_backend("custom_chat")
        return "custom_chat"
...
def get_backend_name() -> str:
    ...
    if optimizer == "qwen_chat" and target == "qwen_chat":
        return "qwen_chat"
    if optimizer == "custom_chat" and target == "custom_chat":
        return "custom_chat"
    ...
    if optimizer == "openai_chat" and target == "minimax_chat":
        return "minimax_chat"
    if optimizer == "openai_chat" and target == "custom_chat":
        return "custom_chat"
...
def chat_optimizer(
    ...
    if get_optimizer_backend() == "custom_chat":
        return _custom.chat_optimizer(
            system=system,
            user=user,
            max_completion_tokens=max_completion_tokens,
            retries=retries,
...
def chat_target(
    ...
    if get_target_backend() == "custom_chat":
        return _custom.chat_target(
            system=system,
            user=user,
            max_completion_tokens=max_completion_tokens,
            retries=retries,
    ...
    if not is_target_chat_backend():
        raise NotImplementedError(
            "chat_target is only supported with target_backend=openai_chat, claude_chat, qwen_chat, minimax_chat, or custom_chat. "
...
def chat_optimizer_messages(
    ...
    *,
    ...
    if get_optimizer_backend() == "custom_chat":
        return _custom.chat_optimizer_messages(
            messages=messages,
            max_completion_tokens=max_completion_tokens,
            retries=retries,
            stage=stage,
...
def chat_target_messages(
    ...
    *,
    ...
    if get_target_backend() == "custom_chat":
        return _custom.chat_target_messages(
            messages=messages,
            max_completion_tokens=max_completion_tokens,
            retries=retries,
            stage=stage,
    ...
    if not is_target_chat_backend():
        raise NotImplementedError(
            "chat_target_messages is only supported with target_backend=openai_chat, claude_chat, qwen_chat, minimax_chat, or custom_chat. "
            "Exec backends are handled in environment-specific rollout code."
        )
...
def get_token_summary() -> dict:
    ...
    for stage, values in minimax_summary.items():
        ...
        summary[stage]["completion_tokens"] += values["completion_tokens"]
        summary[stage]["total_tokens"] += values["total_tokens"]
    custom_summary = _custom.get_token_summary()
    for stage, values in custom_summary.items():
        if stage == "_total":
            continue
        if stage not in summary:
            summary[stage] = values
...
def set_reasoning_effort(effort: str | None) -> None:
    ...
    _custom.set_reasoning_effort(effort)
...
def set_target_deployment(deployment: str) -> None:
    ...
    _custom.set_target_deployment(deployment)
...
def set_optimizer_deployment(deployment: str) -> None:
    ...
    _custom.set_optimizer_deployment(deployment)
