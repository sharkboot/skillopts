...
from . import azure_openai, claude_backend, codex_backend, custom_chat_backend
...
def _backend_module(name: str):
    ...
    if name == "custom_chat":
        return custom_chat_backend
    raise ValueError(f"Unknown backend: {name!r}")


def _all_backend_modules() -> list[Any]:
    return [azure_openai, codex_backend, claude_backend, custom_chat_backend]
...
def set_backend(name: str | None) -> str:
    ...
    if normalized not in {"azure_openai", "codex", "claude", "custom_chat"}:
        valid = ", ".join(sorted({"azure_openai", "codex", "claude", "custom_chat"}))
