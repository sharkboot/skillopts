...
def _role_env(role: str, key: str, default: str) -> str:
    role_key = f"{role.upper()}_CUSTOM_CHAT_{key}"
    generic_key = f"CUSTOM_CHAT_{key}"
    return os.environ.get(role_key) or os.environ.get(generic_key) or default


def _initial_config(role: str) -> CustomChatConfig:
    ...
    return CustomChatConfig(
        ...
        deployment=(
            os.environ.get(f"{role_upper}_CUSTOM_CHAT_MODEL")
            or os.environ.get("CUSTOM_CHAT_MODEL")
...
def _chat_url(config: CustomChatConfig) -> str:
    base = config.base_url.strip().rstrip("/")
    if not base:
        raise RuntimeError(
            "custom_chat base_url is not configured. Set model.custom_chat_base_url, "
            "CUSTOM_CHAT_BASE_URL, or the optimizer/target-specific custom_chat base_url."
        )
...
def _compat_message_from_payload(message: dict[str, Any], choice: dict[str, Any]) -> CompatAssistantMessage:
    ...
    for index, tool_call in enumerate(message.get("tool_calls") or [], start=1):
        ...
        tool_calls.append(
            CompatToolCall(
                id=str(tool_call.get("id") or f"custom_tool_{index}"),
...
def _post_chat_completion(
    ...
    try:
        ...
        raise RuntimeError(f"custom_chat API returned HTTP {e.code}: {body}") from e
    except urllib.error.URLError as e:
        raise RuntimeError(f"custom_chat API request failed: {e}") from e
    try:
        return json.loads(raw)
    except json.JSONDecodeError as e:
        raise RuntimeError(f"custom_chat API returned non-JSON response: {raw[:1000]}") from e
    ...
    *,
    ...
    for attempt in range(retries):
        try:
            ...
            if not choices:
                raise RuntimeError(f"custom_chat API returned no choices: {data}")
            choice0 = choices[0]
    ...
    raise RuntimeError(f"custom_chat call failed after {retries} retries: {last_err}")


def configure_custom_chat(
    *,
    ...
    with _config_lock:
        if base_url is not None:
            os.environ["CUSTOM_CHAT_BASE_URL"] = str(base_url).strip()
        if api_key is not None:
            os.environ["CUSTOM_CHAT_API_KEY"] = str(api_key).strip()
        if model is not None:
            os.environ["CUSTOM_CHAT_MODEL"] = str(model).strip()
        if temperature is not None:
            os.environ["CUSTOM_CHAT_TEMPERATURE"] = str(temperature).strip()
        if timeout_seconds is not None:
            os.environ["CUSTOM_CHAT_TIMEOUT_SECONDS"] = str(timeout_seconds)
        if max_tokens is not None:
            os.environ["CUSTOM_CHAT_MAX_TOKENS"] = str(max_tokens)
        _update_config(
            OPTIMIZER_CONFIG,
            "optimizer",
            ...
            model=optimizer_model if optimizer_model is not None else model,
            temperature=(
        ...
        _update_config(
            ...
            model=target_model if target_model is not None else model,
...
def _update_config(
    ...
    *,
    ...
    if base_url is not None:
        ...
        os.environ[f"{env_prefix}_CUSTOM_CHAT_BASE_URL"] = config.base_url
    if api_key is not None:
        config.api_key = str(api_key).strip()
        os.environ[f"{env_prefix}_CUSTOM_CHAT_API_KEY"] = config.api_key
    if model is not None:
        config.deployment = str(model).strip() or config.deployment
        os.environ[f"{env_prefix}_CUSTOM_CHAT_MODEL"] = config.deployment
    if temperature is not None:
        raw = str(temperature).strip()
        config.temperature = float(raw) if raw else None
        os.environ[f"{env_prefix}_CUSTOM_CHAT_TEMPERATURE"] = raw
    if timeout_seconds is not None:
        raw = str(timeout_seconds).strip()
        if raw:
            config.timeout_seconds = float(raw)
            os.environ[f"{env_prefix}_CUSTOM_CHAT_TIMEOUT_SECONDS"] = raw
    if max_tokens is not None:
        raw = str(max_tokens).strip()
        if raw:
            config.max_tokens = int(raw)
            os.environ[f"{env_prefix}_CUSTOM_CHAT_MAX_TOKENS"] = raw
...
def set_target_deployment(deployment: str) -> None:
    TARGET_CONFIG.deployment = deployment or default_model_for_backend("custom_chat")
    os.environ["TARGET_DEPLOYMENT"] = TARGET_CONFIG.deployment


def set_optimizer_deployment(deployment: str) -> None:
    OPTIMIZER_CONFIG.deployment = deployment or default_model_for_backend("custom_chat")
